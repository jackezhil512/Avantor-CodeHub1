...
function DeleteVWRintegration(){
NavigateToIntegrationMaintenance();
click(fallback(`byXPath("(//div[text()=' Vendor ']/following::mat-select)[1]")`));
click(fallback(`byXPath("//span[text()=' VWR ']/parent::mat-option")`));
click(fallback(`byXPath("//span[text()='Update Results']/ancestor::button")`));
let value = _isVisible(fallback(`byXPath("//img[@src='/assets/icons/ellipsis-icon.svg']/parent::span")`));
log(value);
if(true == value){
...
// JavaScript function to ensure only one VWR entry remains
function ensureSingleVWR() {
   click(fallback(`byXPath("//div[@id='mat-select-value-17']/span/span")`));
    click(fallback(`byXPath("//mat-option[@id='mat-option-276']/span")`));
click(fallback(`byXPath('//app-root/app-avantor-layout/mat-drawer-container/mat-drawer-content/div/div/app-layout/div/app-list/div[3]/div/div/div[4]/div/button[2]/span[1]/span')`));
    wait(200); // Wait for the filter results to load and reflect in the UI    wait(200); // Wait for the filter results to load

 // Assuming each VWR entry can be interacted with through a '.mat-row' selector
    const vwrEntries = findElements('.mat-row'); // Hypothetical function provided by Appvance
    const vwrCount = vwrEntries.length;

    // If more than one entry, start the process to delete the extras
    if (vwrCount > 1) {
        // Loop through the extra entries, starting from the last one
        for (let i = vwrCount - 1; i >= 1; i--) { // Start from the last entry to avoid indexing issues during deletion
            // Click the three-dot menu of the current VWR entry
            const menuButton = vwrEntries[i].findElement(`byXPath('//app-root/app-avantor-layout/mat-drawer-container/mat-drawer-content/div/div/app-layout/div/app-list/div[4]/div/avantor-table/div[2]/table/tbody/tr[1]/td[15]/button/span[1]/img').xy(0.44, 0.57 )`); // Adjust selector to target the three-dot menu correctly
            menuButton.click();
            wait(500); // Wait 500 ms for the menu options to appear

            // Click "Delete"
            const deleteOption = findElement(`byXPath("//div[@id='mat-menu-panel-49']/div/button[2]")`); // Adjust selector to target the "Delete" option
            deleteOption.click();
            wait(500); // Wait 500 ms for the confirmation dialog

            // Confirm the deletion
            const confirmYesButton = findElement(`byXPath("//mat-dialog-container[@id='mat-dialog-1']/app-delete-integration-model/div[2]/mat-dialog-actions/button[2]/span[1]")`); // Adjust selector for confirmation button
            confirmYesButton.click();
            wait(500); // Wait 500 ms for the deletion to complete before proceeding to the next
        }
        console.log(`${vwrCount - 1} extra VWR entries deleted, only one remains.`);
    } else if (vwrCount === 1) {
        console.log("Only one VWR entry is present, no deletion needed.");
    } else {
        console.log("No VWR entries found, please check the filters or data.");
    }
}

// Helper function to interact with the vendor dropdown and apply filters
function selectVendor(VWR) {
    // Simulate selecting a vendor from the dropdown. Assume 'setDropdownValue' is a provided method to set the dropdown value.
    setDropdownValue(`byXPath("//mat-option[@id='mat-option-276']/span")`, VWR);

    // Simulate clicking the 'Update Results' button. Assume 'clickButton' is a provided method to simulate button clicks.
    clickButton(`byXPath('//app-root/app-avantor-layout/mat-drawer-container/mat-drawer-content/div/div/app-layout/div/app-list/div[3]/div/div/div[4]/div/button[2]/span[1]/span')`);

    // Wait for the results to update after applying the filter, assuming 'Wait' is a correct method in your environment.
    Wait(500);
}
